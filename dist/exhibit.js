...
$('eyes').onclick=()=>{state.eyes=!state.eyes;setOptics(state.eyes);$('eyes').setAttribute('aria-pressed',state.eyes);$('eyes').querySelector('i').textContent=state.eyes?'ON':'OFF';};
...
function setOptics(on){if(!eyeGroup)return;for(const child of eyeGroup.children){if(child.isPointLight)child.intensity=on?.7:0;if(child.isMesh)child.material.color.set(on?0x33aaff:0x000000);}eyeGroup.visible=state.progress<.04;}
function applyMotion(){const eased=state.progress*state.progress*(3-2*state.progress);const sample=(24+eased*90)/2,index=Math.floor(sample),alpha=sample-index;
 ...
 if(eyeGroup)eyeGroup.visible=state.progress<.04;
}
