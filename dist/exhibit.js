...
function scanShader(shader){
 shader.uniforms.scanFrom=scan.from;shader.uniforms.scanTo=scan.to;shader.uniforms.scanHeight=scan.height;shader.uniforms.scanActive=scan.active;shader.uniforms.colorProgress=colorProgress;shader.uniforms.colorHeight=colorHeight;
...
function finish(color,name){tint.value.set(color);tintAmount.value=name==='Racing yellow'?0:1;colorProgress.value=0;colorHeight.value=4;$('color-name').textContent=name;document.querySelectorAll('[data-color]').forEach(b=>b.setAttribute('aria-pressed',b.dataset.name===name));}
...
async function load(){const draco=new DRACOLoader();draco.setDecoderPath('/vendor/draco/');const loader=new GLTFLoader();loader.setDRACOLoader(draco);const [gltf,motion]=await Promise.all([loader.loadAsync('/assets/guardian.glb',e=>{$('load-progress').style.width=`${e.total?Math.min(95,e.loaded/e.total*95):45}%`;}),fetch('/assets/guardian-motion.json').then(r=>{if(!r.ok)throw Error('Motion data unavailable');return r.json();})]);
 ...
 #include <opaque_fragment>`);shader.uniforms.armorTint=tint;shader.uniforms.armorAmount=tintAmount;shader.fragmentShader='uniform vec3 armorTint; uniform float armorAmount; uniform float colorProgress; uniform float colorHeight;\n'+shader.fragmentShader;shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
 ...
 float colorBand=smoothstep(colorHeight-.28,colorHeight+.32,scanWorldY)*colorProgress; diffuseColor.rgb=mix(diffuseColor.rgb,armorTint*max(diffuseColor.r,diffuseColor.g),yellowMask*armorAmount*colorBand);`);};mat.customProgramCacheKey=()=> 'guardian-scan-hologram-v2';});
