...
import {pass, reflector, screenUV, blendOverlay, uniform, vec3} from 'three/tsl';
...
const floorMaterial=new THREE.MeshPhongNodeMaterial({color:'#080a0d',shininess:18});const floorReflection=reflector({resolutionScale:.32});floorReflection.target.rotateX(-Math.PI/2);floorReflection.target.scale.set(10,10,10);floorReflection.target.position.y=-.007;scene.add(floorReflection.target);floorMaterial.emissiveNode=floorReflection.mul(.22);const floor=new THREE.Mesh(new THREE.PlaneGeometry(100,100),floorMaterial);floor.rotation.x=-Math.PI/2;floor.position.y=-.007;floor.receiveShadow=true;scene.add(floor);
...
const scenePass=pass(scene,camera),sceneColor=scenePass.getTextureNode();const composer=new THREE.PostProcessing(renderer);composer.outputNode=sceneColor;
...
async function load(){const draco=new DRACOLoader();draco.setDecoderPath('/vendor/draco/');const loader=new GLTFLoader();loader.setDRACOLoader(draco);const [gltf,motion]=await Promise.all([loader.loadAsync('/assets/guardian.glb',e=>{$('load-progress').style.width=`${e.total?Math.min(95,e.loaded/e.total*95):45}%`;}),fetch('/assets/guardian-motion.json').then(r=>{if(!r.ok)throw Error('Motion data unavailable');return r.json();})]);
 ...
 for(const part of parts){const wireMat=new THREE.MeshBasicNodeMaterial({transparent:true,wireframe:true,depthWrite:false,toneMapped:false});wireMat.colorNode=vec3(.08,2.4,1.5);part.wire=new THREE.Mesh(part.mesh.geometry,wireMat);part.wire.visible=false;part.wire.raycast=()=>{};part.mesh.add(part.wire);}
