...
const state={ready:false,progress:0,target:0,separation:0,explode:0,eyes:false,cameraAdjusted:false};
...
const scanGLSL=`uniform float scanFrom,scanTo,scanHeight,scanActive; varying float scanWorldY; varying vec3 scanWorldPosition;
float wireRegion(){return mix(scanFrom,scanTo,step(scanHeight,scanWorldY));}`;
function scanShader(shader){
 shader.uniforms.scanFrom=scan.from;shader.uniforms.scanTo=scan.to;shader.uniforms.scanHeight=scan.height;shader.uniforms.scanActive=scan.active;
 shader.vertexShader='varying float scanWorldY; varying vec3 scanWorldPosition;\n'+shader.vertexShader;
 shader.vertexShader=shader.vertexShader.replace('#include <project_vertex>','#include <project_vertex>\nscanWorldPosition=(modelMatrix*vec4(transformed,1.0)).xyz;scanWorldY=scanWorldPosition.y;');
 shader.fragmentShader=scanGLSL+'\n'+shader.fragmentShader;
}
$('wireframe').onclick=()=>{
...
try{renderer=new THREE.WebGLRenderer({canvas:$('canvas'),antialias:true,powerPreference:'high-performance'});}catch(e){fail(e);throw e;}
renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=.85;
...
function form(target){state.target=target;state.explode=0;state.cameraAdjusted=false;focus=null;document.body.classList.remove('inspecting');$('inspection').hidden=true;if(target===1&&state.progress<.1){camera.position.set(4,2.9,7.6);controls.target.set(0,1.72,0);controls.update();}}
...
function addHotspot(name,title,copy,position){const mesh=parts.find(p=>p.mesh.name===name)?.mesh;if(!mesh)return;const local=mesh.worldToLocal(new THREE.Vector3(...position));const button=document.createElement('button');button.className='hotspot';button.textContent='+';button.dataset.label=title.toUpperCase();button.setAttribute('aria-label',`Inspect ${title}`);$('hotspots').append(button);const spot={mesh,local,button};spots.push(spot);button.onclick=()=>{const point=mesh.localToWorld(local.clone());focus={position:point.clone().add(new THREE.Vector3(.5,.12,1.6)),target:point};controls.autoRotate=false;document.body.classList.add('inspecting');$('inspection').hidden=false;$('inspect-title').textContent=title;$('inspect-copy').textContent=copy;};}
async function load(){const draco=new DRACOLoader();draco.setDecoderPath('/vendor/draco/');const loader=new GLTFLoader();loader.setDRACOLoader(draco);const [gltf,motion]=await Promise.all([loader.loadAsync('/assets/guardian.glb',e=>{$('load-progress').style.width=`${e.total?Math.min(95,e.loaded/e.total*95):45}%`;}),fetch('/assets/guardian-motion.json').then(r=>{if(!r.ok)throw Error('Motion data unavailable');return r.json();})]);
 scene.add(gltf.scene);gltf.scene.traverse(mesh=>{if(!mesh.isMesh)return;const frames=motion.parts[mesh.name];if(!frames)return;mesh.castShadow=true;mesh.receiveShadow=true;mesh.frustumCulled=false;mesh.geometry.computeBoundingBox();parts.push({mesh,frames,center:mesh.geometry.boundingBox.getCenter(new THREE.Vector3())});mesh.material=mesh.material.clone();const mat=mesh.material;mat.polygonOffset=true;mat.polygonOffsetFactor=1;mat.polygonOffsetUnits=1;mat.onBeforeCompile=shader=>{scanShader(shader);shader.fragmentShader=shader.fragmentShader.replace('#include <opaque_fragment>',`float region=wireRegion();
 if(region>.5 && (scanActive<.5 || abs(scanWorldY-scanHeight)>.035)) discard;
 float laser=exp(-pow((scanWorldY-scanHeight)/.022,2.0))*scanActive;
 outgoingLight+=vec3(.15,5.0,9.0)*laser;
 #include <opaque_fragment>`);shader.uniforms.armorTint=tint;shader.uniforms.armorAmount=tintAmount;shader.fragmentShader='uniform vec3 armorTint; uniform float armorAmount;\n'+shader.fragmentShader;shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
 ...
 diffuseColor.rgb=mix(diffuseColor.rgb,armorTint*max(diffuseColor.r,diffuseColor.g),yellowMask*armorAmount);`);};mat.customProgramCacheKey=()=> 'guardian-scan-hologram-v2';});
 for(const part of parts){const wireMat=new THREE.MeshBasicMaterial({color:new THREE.Color(.08,2.4,1.5),transparent:true,opacity:.16,blending:THREE.AdditiveBlending,side:THREE.DoubleSide,depthWrite:false,toneMapped:false});wireMat.onBeforeCompile=shader=>{scanShader(shader);shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>','#include <color_fragment>\nif(wireRegion()<.5) discard;\nvec3 grid=scanWorldPosition/.055;\nvec3 fw=max(fwidth(grid),vec3(.0001));\nvec3 lines=abs(fract(grid-.5)-.5)/fw;\nvec3 normal=normalize(cross(dFdx(scanWorldPosition),dFdy(scanWorldPosition)));\nlines+=step(vec3(.88),abs(normal))*100.;\nfloat line=1.-smoothstep(.25,1.05,min(lines.x,min(lines.y,lines.z)));\nif(line<.01)discard;\ndiffuseColor.a*=line;');};wireMat.customProgramCacheKey=()=> 'guardian-wire-hologram-v2';part.wire=new THREE.Mesh(part.mesh.geometry,wireMat);part.wire.visible=false;part.wire.raycast=()=>{};part.mesh.add(part.wire);}
...
function resize(){const w=$('viewport').clientWidth,h=$('viewport').clientHeight;renderer.setSize(w,h,false);composer.setSize(w,h);camera.aspect=w/h;camera.setViewOffset(w,h,w>760?-160:0,0,w,h);camera.updateProjectionMatrix();}new ResizeObserver(resize).observe($('viewport'));resize();
let last=performance.now();function tick(now){requestAnimationFrame(tick);const dt=Math.min((now-last)/1000,.05);last=now;if(state.ready){state.separation=THREE.MathUtils.damp(state.separation,state.explode,7,dt);if(Math.abs(state.separation-state.explode)<.001)state.separation=state.explode;if(state.target===1&&!state.cameraAdjusted&&state.progress>.62){state.cameraAdjusted=true;focus={position:new THREE.Vector3(4,2.5,6.5),target:new THREE.Vector3(0,.7,0)};}if(state.separation<.01&&state.progress!==state.target)state.progress=THREE.MathUtils.clamp(state.progress+Math.sign(state.target-state.progress)*Math.min(dt/3,Math.abs(state.target-state.progress)),0,1);applyMotion();const moving=Math.abs(state.progress-state.target)>.001;for(const [id,value]of [['robot',0],['vehicle',1]])$(id).setAttribute('aria-pressed',state.target===value);$('transform-label').textContent=state.target?'Transform to robot':'Transform to vehicle';$('explode').disabled=moving;$('separation').disabled=moving;$('eyes').disabled=state.progress>.04;$('explode').setAttribute('aria-pressed',state.explode>.01);$('assembly-label').textContent=state.explode>.01?'Reassemble':'Exploded view';$('separation').value=Math.round(state.separation*100);$('separation-value').textContent=`${Math.round(state.separation*100)}%`;$('status').textContent=moving?'TRANSFORMATION IN PROGRESS':state.separation>.01?'COMPONENT EXPLORATION':state.target?'VEHICLE SYSTEMS READY':'GUARDIAN SYSTEMS READY';$('sequence-label').textContent=moving?'MECHANICAL SEQUENCE':state.target?'VEHICLE STANDBY':'GUARDIAN STANDBY';$('sequence-progress').style.width=`${state.progress*100}%`;$('sequence-number').textContent=`${String(Math.round(state.progress*100)).padStart(2,'0')} / 100`;$('form-index').textContent=state.target?'02 / VEHICLE FORM':'01 / ROBOT FORM';$('part-count').textContent=state.target?'VEHICLE CONFIGURATION':'177 ORIGINAL ROBOT COMPONENTS';}
 if(focus){const t=reduced?1:1-Math.exp(-dt*5);camera.position.lerp(focus.position,t);controls.target.lerp(focus.target,t);if(camera.position.distanceTo(focus.position)<.005)focus=null;}controls.update();scene.updateMatrixWorld(true);updateScan(dt);for(const spot of spots){const p=spot.mesh.localToWorld(spot.local.clone()).project(camera);spot.button.hidden=state.progress>.01||state.separation>.01||p.z>1||camera.position.z<0;spot.button.style.left=`${(p.x*.5+.5)*$('viewport').clientWidth}px`;spot.button.style.top=`${(-p.y*.5+.5)*$('viewport').clientHeight}px`;}composer.render();}requestAnimationFrame(tick);load().catch(fail);
