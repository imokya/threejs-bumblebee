...
function addHotspot(name,title,copy,position){const mesh=parts.find(p=>p.mesh.name===name)?.mesh;if(!mesh)return;const local=mesh.worldToLocal(new THREE.Vector3(...position));const button=document.createElement('button');button.className='hotspot';button.textContent='+';button.dataset.label=title.toUpperCase();button.setAttribute('aria-label',`Inspect ${title}`);$('hotspots').append(button);const spot={mesh,local,button};spots.push(spot);button.onclick=()=>{const point=mesh.localToWorld(local.clone());focus={position:point.clone().add(new THREE.Vector3(1.25,.35,3.15)),target:point};controls.autoRotate=false;document.body.classList.add('inspecting');$('inspection').hidden=false;$('inspect-title').textContent=title;$('inspect-copy').textContent=copy;};}
