...
function scanShader(shader){
 shader.uniforms.scanFrom=scan.from;shader.uniforms.scanTo=scan.to;shader.uniforms.scanHeight=scan.height;shader.uniforms.scanActive=scan.active;shader.uniforms.colorProgress=colorProgress;shader.uniforms.colorHeight=colorHeight;shader.uniforms.colorStart=colorStart;shader.uniforms.amountStart=amountStart;
...
const groundReflector=new ReflectorForSSRPass(new THREE.PlaneGeometry(7,7),{clipBias:.003,textureWidth:512,textureHeight:512,color:0x161e24});groundReflector.rotation.x=-Math.PI/2;groundReflector.position.y=-.002;scene.add(groundReflector);
...
const ssrPass=new SSRPass({renderer,scene,camera,width:512,height:512,groundReflector});ssrPass.thickness=.08;ssrPass.maxDistance=2.2;ssrPass.opacity=.22;ssrPass.fresnel=.18;const composer=new EffectComposer(renderer);composer.addPass(ssrPass);composer.addPass(new RenderPass(scene,camera));composer.addPass(new UnrealBloomPass(new THREE.Vector2(1,1),.15,.4,2));composer.addPass(new OutputPass());
...
const tint={value:new THREE.Color('#ffc43b')},tintAmount={value:0},colorStart={value:new THREE.Color('#ffc43b')},amountStart={value:0};
const colorProgress={value:0},colorHeight={value:4};
