...
async function load(){const draco=new DRACOLoader();draco.setDecoderPath('/vendor/draco/');const loader=new GLTFLoader();loader.setDRACOLoader(draco);const [gltf,motion]=await Promise.all([loader.loadAsync('/assets/guardian.glb',e=>{$('load-progress').style.width=`${e.total?Math.min(95,e.loaded/e.total*95):45}%`;}),fetch('/assets/guardian-motion.json').then(r=>{if(!r.ok)throw Error('Motion data unavailable');return r.json();})]);
 ...
 const region=mix(scan.from,scan.to,step(scan.height,positionWorld.y));
 const wireMat=new THREE.MeshBasicNodeMaterial({transparent:true,blending:THREE.AdditiveBlending,side:THREE.DoubleSide,depthWrite:false,toneMapped:false});
 ...
 part.wire=new THREE.Mesh(part.mesh.geometry,wireMat);part.wire.visible=false;part.wire.raycast=()=>{};part.mesh.add(part.wire);
 }
...
function resize(){const w=$('viewport').clientWidth,h=$('viewport').clientHeight;renderer.setSize(w,h,false);camera.aspect=w/h;camera.setViewOffset(w,h,w>760?-160:0,0,w,h);camera.updateProjectionMatrix();}new ResizeObserver(resize).observe($('viewport'));resize();
